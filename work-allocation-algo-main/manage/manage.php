...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $insertQuery = "INSERT INTO `tbl_ticket`(`ticket_name`, `ticket_type`, `ticket_priority`, `ticket_difficulty`, `ticket_deadline`,`dev_type`,`dev_id`) VALUES ('$ticket_name', '$ticket_type', '$ticket_priority', '$ticket_difficulty', '$ticket_deadline','$ticket_dev',0)";

    $con->query($insertQuery);
        echo "Ticket created successfully!";
	
        // Update dev_load by incrementing it by 1
        $updateDevLoad = $con->query("UPDATE tbl_dev SET dev_load = dev_load + 1 WHERE dev_id = $did");

        // Assign the same developer to the ticket with the highest ticket_id
		$updateTicket = $con->query("UPDATE `tbl_ticket` SET `dev_id` = $did, `ticket_status` = `ticket_status` + 1 WHERE ticket_id = (SELECT MAX(`ticket_id`) FROM `tbl_ticket`)");
